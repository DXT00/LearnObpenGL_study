...
#define UGL_REQUIRED_VOID			{ assert(0); }
#define UGL_REQUIRED(ReturnValue)	{ assert(0); return (ReturnValue); }
#define UGL_OPTIONAL_VOID			{ }
#define UGL_OPTIONAL(ReturnValue)	{ return (ReturnValue); }
struct FPlatformOpenGLDevice;
struct FPlatformOpenGLContext;
...
public:
	...
	static void SetupDefaultGLContextState(const std::string& ExtensionsString) {};
	...
	static FORCEINLINE GLint GetMaxComputeTextureImageUnits()	{ assert(MaxComputeTextureImageUnits != -1); return MaxComputeTextureImageUnits; }
	...
	static FORCEINLINE uint64_t GetVideoMemorySize()				{ return 0; }

	static FORCEINLINE bool IsDebugContent()					{ return false; }
	static FORCEINLINE void InitDebugContext()					{ }

	static FORCEINLINE int32_t GetReadHalfFloatPixelsEnum() UGL_REQUIRED(0)
	...
	static FORCEINLINE void GetQueryObject(GLuint QueryId, EQueryMode QueryMode, uint64_t *OutResult) UGL_OPTIONAL_VOID
	...
	static FORCEINLINE void PushGroupMarker(const char* Name) UGL_OPTIONAL_VOID
	static FORCEINLINE void PopGroupMarker() UGL_OPTIONAL_VOID
	static FORCEINLINE void LabelObject(GLenum Type, GLuint Object, const char* Name) UGL_OPTIONAL_VOID
	static FORCEINLINE GLsizei GetLabelObject(GLenum Type, GLuint Object, GLsizei BufferSize, char* OutName) UGL_OPTIONAL(0)
	...
	static FORCEINLINE void* MapBufferRange(GLenum Type, uint32_t InOffset, uint32_t InSize, EResourceLockMode LockMode) UGL_REQUIRED(NULL)
	static FORCEINLINE void UnmapBufferRange(GLenum Type, uint32_t InOffset, uint32_t InSize) UGL_REQUIRED_VOID
	...
	static FORCEINLINE const char* GetStringIndexed(GLenum Name, GLuint Index) UGL_REQUIRED(NULL)
	...
	static FORCEINLINE std::string GetAdapterName() UGL_REQUIRED((""))
	...
	static FORCEINLINE void ClearBufferData(GLenum Target, GLenum InternalFormat, GLenum Format, GLenum Type, const uint32_t* Data) UGL_REQUIRED_VOID
	...
	static FORCEINLINE Format PreferredPixelFormatHint(Format PreferredPixelFormat)
	{
		// Use a default pixel format if none was specified	
		if (PreferredPixelFormat == Format::UNKNOWN)
		{
			PreferredPixelFormat = Format::BGRA8_UNORM;
		}
